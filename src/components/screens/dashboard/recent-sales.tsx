          ...
          {sales.map((sale) => (
            <div key={sale.id} className='flex w-full items-center overflow-hidden'>
              ...
              <div className='mx-4 space-y-1 overflow-hidden'>
                <p className='truncate text-sm font-medium leading-none dark:text-light-500 text-dark-500'>{sale.name}</p>
                <p className='truncate text-sm text-muted-foreground'>{sale.email}</p>
              </div>
              <div className='text-sm font-medium sm:text-base ltr:ml-auto rtl:mr-auto dark:text-light-500 text-dark-500'>
                {sale.amount}
              </div>
            </div>
          ))}
